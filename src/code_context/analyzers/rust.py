...
class RustAnalyzer(BaseAnalyzer):
    ...
    def __init__(self):
        super().__init__("rust", tree_sitter_rust)
    ...
    def analyze(self, file_path: Path) -> Optional[FileAnalysis]:
        ...
        summary = f"Rust file with {type_summary}"
    ...
    def _collect_symbols(self, node: ts.Node, source: bytes, symbols: list[Symbol], parent: Optional[Symbol] = None):
        ...
        if node.type in symbol_map:
            ...
            if name:
                ...
                return_type = self._get_return_type(node, source) if sym_type == "function" else None
                ...
                if sym_type in ("struct", "enum", "impl", "trait"):
                    for child in node.children:
                        if child.type in ("field_declaration_list", "enum_variant_list", "declaration_list"):
                            for list_child in child.children:
                                self._collect_symbols(list_child, source, symbols, parent=sym)
                return
    ...
    def _get_symbol_name(self, node: ts.Node, source: bytes) -> str:
        """Extract symbol name from Rust declaration."""
        for child in node.children:
