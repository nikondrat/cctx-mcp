...
def _get_analyzer_for_file(file_path: Path):
    ext = file_path.suffix.lower()
    lang = _ANALYZER_LANG_MAP.get(ext)
    if lang is None:
        return None
    try:
        if lang == "python":
            ...
            from analyzers.dart import DartAnalyzer
            return DartAnalyzer()
    except Exception:
        return None
    return None
...
def _symbols_to_chunks(symbols, rel: str, fhash: str, lines: list[str]) -> list[Chunk]:
    from analyzers.base import Symbol

    chunks: list[Chunk] = []
    for sym in symbols:
        if not isinstance(sym, Symbol):
            continue
        snippet = "\n".join(lines[sym.start_line - 1:sym.end_line]).strip()[:_MAX_SNIPPET_CHARS]
        if snippet:
            cid = hashlib.md5(f"{rel}:{sym.start_line}:{sym.name}".encode()).hexdigest()[:12]
            chunks.append(Chunk(chunk_id=cid, file=rel, line_start=sym.start_line, symbol=sym.name, snippet=snippet, file_hash=fhash))
        for child in sym.children:
            child_snippet = "\n".join(lines[child.start_line - 1:child.end_line]).strip()[:_MAX_SNIPPET_CHARS]
            if child_snippet:
                full_name = f"{sym.name}.{child.name}"
                cid = hashlib.md5(f"{rel}:{child.start_line}:{full_name}".encode()).hexdigest()[:12]
                chunks.append(Chunk(chunk_id=cid, file=rel, line_start=child.start_line, symbol=full_name, snippet=child_snippet, file_hash=fhash))
    return chunks
...
def _extract_chunks(file_path: Path, project_path: Path) -> list[Chunk]:
    ...
    Uses tree-sitter analyzers for supported languages, falls back to regex.
    """
    try:
    ...
    analyzer = _get_analyzer_for_file(file_path)
    if analyzer is not None:
        tree = analyzer.parse_file(file_path)
        if tree is not None:
            source = text.encode("utf-8")
            symbols = analyzer.find_symbols(tree.root_node, source)
            if symbols:
                chunks = _symbols_to_chunks(symbols, rel, fhash, lines)

    # Fallback to regex if analyzer returned nothing
    if not chunks:
        chunks = _regex_extract_chunks(lines, text, rel, fhash)
