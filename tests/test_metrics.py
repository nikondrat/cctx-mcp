...
def _metrics() -> tuple[Metrics, Path]:
    tmp = Path(tempfile.mkdtemp())
    return Metrics(events_path=tmp / "events.jsonl"), tmp
...
class TestMetrics(unittest.TestCase):
    ...
    def test_get_daily_trend_returns_recent_days(self):
        ...
        (tmp / "events.jsonl").touch()
        Metrics._DEFAULT_EVENTS_DIR  # ensure const exists
        ...
        self.assertEqual(trend[1].get("tools"), {})
